...
The **Todo Service** is a full-stack task management CRUD application built with React (frontend) and Node.js/Express (backend) with in-memory storage. It will be containerized (Docker) and deployed to **AWS ECS Fargate** behind an Application Load Balancer, with logs and metrics collected via CloudWatch and Container Insights. The frontend runs on port 80 (nginx proxy) and the backend API runs on port 4000.
